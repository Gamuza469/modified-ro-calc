{ // AdditionalInfoList (top-right) [KAKUTYOU_NAME]
EXTENDED_INFO_NAME = [
 [0,"","-"]
,[1,"","Heal Amount"]
,[2,"","Increased HP Recovery"]
,[3,"","Increased SP Recovery"]
,[4,"","Weight Limit"]
,[5,"","Elemental Resistance"]
,[6,"","Race Resistance"]
,[7,"","Status Resistance"]
,[8,"","Others Resistance"]
,[9,"","Cast and Delay"]
,[10,"","Experience Calculator"]
];
}

{ // MonsterSortOptions [SORT_NAME]
SORT_NAME = [
 ["","Alphabetical"]
,["","By Element"]
,["","By Race"]
,["","Base Level"]
,["","100% HIT"]
,["","95% Flee"]
,["","By Base Exp"]
,["","By Job Exp"]
,["","By Max Atk"]
];
}

{ // Delay between actions List [ATKTIME_NAME]
ATKTIME_NAME = [
 [10,"","0.1s (10 per second)"]
,[15,"","0.15s"]
,[20,"","0.2s (5 per second)"]
,[25,"","0.25s (4 per second)"]
,[30,"","0.3s"]
,[33,"","0.33s (3 per second)"]
,[40,"","0.4s"]
,[50,"","0.5s (2 per second)"]
];
}

{ // CombatResultTexts [SubName]
SubName = [
 ["","%"]
,["","sec"]
,["","Damage"]
,["","Critical Damage"]
,["","Critical Rate"]
,["","Over 10000 Hits"]
,["","Too High to Calculate"]
,["","Immeasurable"]
,["","X"]
,["","Cast Time"]
,["","Off"]//10
,["","On"]
];
}

{ // Global Labels [WordData]
WordData = [
 [" "," "]//0
,["","Base Stats"]
,["","Auto Adjust Base Level"]
,["","LV"]
,["","JobLV"]
,["","Class"]
,["","STR"]
,["","AGI"]
,["","VIT"]
,["","INT"]
,["","DEX"] //10
,["","LUK"]
,["","Remaining Status Points"]
,["","Extended Info"]
,["","Max HP"]
,["","Max SP"]
,["","DEF"]
,["","MDEF"]
,["","HIT"]
,["","FLEE"]
,["�?","Perfect Dodge"]//20
,["","Critical"]
,["","MATK"]
,["","ASPD"]
,["","HP Regen"]
,["","SP Regen"]
,["","Speed Potion: "]
,["","Weapon Type: "]
,["","Weapon & Cards"]
,["","Weapon: "]
,["","Attribute: "]//30
,[0,0]
,["","Armor & Cards"]
,["","Passive / Duration Skills"]
,["","Supportive / Party Skills"]
,["","Show "]//35
,["","Attack Skills: "]
,["","Monster: "]
,["","HP"]
,["","BaseExp"]
,["","ATK"]//40
,["","&nbsp;~ "]
,["","JobExp"]
,["","DEF"]
,["","Race"]
,["","MDEF"]
,["","Element"]
,["","Perfect Hit"]
,["","Size"]
,["","95% Dodge"]
,["","Monster Debuffs"]//50
,["","Debuffs"]
,["","Combat Results (alpha)"]
,["","Hit Ratio"]
,["","Dodge Ratio"]
,["","Minimum Damage"]//55
,["","Average Damage"]
,["","Maximum Damage"]
,["","Damage Per Second"]
,["","Minimum Number of Hits"]
,["","Average Number of Hits"]//60
,["","Maximum Number of Hits"]
,["��","Average Battle Duration"]
,["","Base Exp Per Hit"]
,["","Job Exp Per Hit"]
,["","Average Damage Recieved"]
,["","Average Damage Recieved (w/dodge)"]
,["","%"]
,["","%"]
,["","Music and Dance Skills"]
,["","Map selection"]//70
,["","Guild Skills"]//
,["","This column doesn't correspond to save."]
,["","Battle Chant Effects"]
,[0,]
,["","Miscellaneous Effects"]
,[0,]
,["","Minimum Delay Between Active Skills"]
,["","Adopted"]
,["","Status Items (Foods/Box)"]
,["","Item Data"]//80
,["",""]//*gO(u
,["","Monster Buffs"]
,["","Buffs"]
,["","Save Data Name"]
,["","Card"]//85
,["","bottle containing Deadly Poison"]
,["","#EX"]
,["","Refine: "]
,["","Left: "]
,["","Right:"]//90 *gO(u
,["","Top 10 Rank"]
,["","Card Shortcuts (Left)"]
,["","Unarmed (or Shield)"]
,["","Card Weight: "]
,["","Number of Hits: "]//95
,["","Poison React Level: "]
,["�?","Remaining SP: "]
,["�?","Remaining HP: "]
,["","Number of Hits(Average): "]
,["","Cell to Enemy: "]//100
,["","Andrenaline Rush"]
,["","Spirit Spheres"]
,["","Magnum Break Bonus"]
,["","Aloevera"]
,["","<Font size=2>Additional Buffs Found Below</Font>"]//105
,["","Bard's "]
,["","Dancer's "]
,["","Controller's Stats: "]
,["","Status is added by capitation."]
,["","  <B>[ Active ]</B>"]//110
,["","Battle Orders"]
,["","Great Leadership"]
,["","Wounds of Glory"]
,["","Soul of Cold"]
,["","Sharp Hawk Eyes"]//115
,["","All Stats+20"]
,["","HP+100%"]
,["","SP+100%"]
,["","ATK+100%"]
,["","HIT+50 & FLEE+50"]//120
,["","Damage reducing by half"]
,["","Murderer Bonus"]
,["","Self "]
,["","Sesame Pastry"]
,["","Honey Pastry"]//125
,["","Rainbow Cake"]
,["","Box of Resentment"]
,["","Box of Drowsiness"]
,["","Coldproof Potion"]
,["","Earthproof Potion"]//130
,["","Fireproof Potion"]
,["","Thunderproof Potion"]
,["","Food"]
,["","Magic Scroll/Yggdrasil Leaf/Etc"]
,["","Internet Cafe's All Stats+3"]//135
,[""," (Non Undead)"]
,[""," (Demon/Undead)"]
,["",""]
,["","<BR>(3x Atk, 2x Hit)"]
,["","<BR>(2x Flee)"]//140
,["","Required Int/Lv for next bonus: "]
,["","Not Available for this Class"]
,["","Current Base Exp"]
,["","Current Job Exp"]
,["�?","Regen"]//145
,["�?","Weight Limit"]
,["","Total Weight of Equipment"]
,["","Status minus Resistance"]
,["","(This system is in testing)"]
,["","Boss Monster"]//150
,["","Ranged Attack"]
,["","Nomal Monster"]
,[""," Resistance"]
,["","Cast Time"]
,["��","After Cast Delay"]//155
,["","Next Base Level up: "]
,["","Next Job Level up: "]
,[""," need "]
,["",""]
,["","Slot"]//160
,["","Min Lv"]
,["","Weight"]
,["","Weapon Lv"]
,["","Additional Buffs & Debuffs"]
,["","Save"]//165
,["","Save URL"]
,["","Calculate"]
,["�?","Delete Save Data"]
,["","All Stats"]
,["","ASPD"]//170
,["","CRIT"]
,["","LUCKY"]
,[""," Element Weapon"]
,["","Completely bypasses defence on the target"]
,["","Completely bypasses defence on the target"]//175
,["","Attack power of the weapon increases against enemies with high defence"]
,["","Reduces your defence to 1/"]
,["","Ranged damage"]
,["","Boss"]
,[""," size monsters"]//180
,["�?"," type monsters"]
,[""," element monsters"]
,["","Damage on "]
,["","Resistance to "]
,[""," type monsters"]//185
,[""," element attacks"]
,["","Critical Damage"]
,["","After cast delay"]
,["","Resistance to [Boss] type monsters"]
,["","Resistance to ranged attacks"]//190
,["","Resistance to normal monsters"]
,[0,0]
,[0,0]
,["","Damage on goblin monsters"]
,["","Damage on kobold monsters"]//195
,["","Damage on orc monsters (with the exception of Orc Lord and Orc Hero)"]
,["","Damage on golem monsters"]
,["","Lowers your defence rate "]
,["","Increases your chance to hit all targets by a fixed "]
,["",""]//200
,["","(Staff Type)"]
,["","[Crit] on "]
,["�?"," type monsters "]
,["","Experience from "]
,["�?"," type monsters "]//205
,["","When attacking, adds a "]
,["","% chance to inflict ["]
,["","] on the enemy"]
,["","Status effect "]
,[""," resistance +"]//210
,[""," size monsters by "]
,["","Unrefinable"]
,["","Cannot be broken"]
,["","Two-Handed Staff."]
,["","Armor becomes "]//215
,[""," element"]
,["","Allows usage of the skill ["]
,["","."]
,[""," chance to cast the skill ["]
,["","."]//220
,["","%"]
,["","there is a "]
,["","] damage "]
,[""," Reduction"]
,["","No Peco"]//225
,["","Mastery "]
,["","None"]
,["","Regular AR"]
,["","Full AR"]
,["","Scroll"]//230
,["","Double "]
,["","Triple "]
,["","Quadruble "]
,["","Buche de Noel"]
,["","Rune Strawberry Cake"]//235
,["","Schwartzwald Pine Jubilee"]
,["","Arunafeltz Desert Sandwich"]
,["","ASPD Potions"]
,["","Increase HP Potion"]
,["","Increase SP Potion"]//240
,["","Distilled Fighting Spirit"]
,["","Durian"]
,["","Celermine Juice"]
,["","Guarana Candy"]
,["","Lucky Rice Cake"]//245
,["","Military Ration B"]
,["","Military Ration C"]
,["","Tasty Pink Ration"]
,["","Tasty White Ration"]
,["","Vitata500"]//250
,["","No Dragon"]
,["","Cart Weight"]
,["","VIP Buffs"]
,["","Fixed Cast Time"]
,["","Variable Cast Time"]//255
,["","Skill Reuse Delay"]
,["","Damage with "]
,[""," element magic "]
,["","Abrasive"]
,["","Holy Elemental Scroll"] //260
,["","Undead Elemental Scroll"]
];
}

{ // AutoCastTrigger [AUTOSPELL_NAME]
AUTOSPELL_NAME = [
 [0,0]
,["","When performing a physical attack, "]
,["","When performing a short range physical attack, "]
,["","When performing a long range physical attack, "]
,["","When performing a magical attack, "]
,["","When attacking, "]
,["","When recieving physical damage, "]
,["","When recieving short range physical damage, "]
,["","When recieving long range physical damage, "]
,["","When recieving magical damage, "]
,["","When recieving physical or magical damage, "]
];
}

{ // Autospell SuccessLabels [AUTOSPELL_NAME]
AUTOSPELL_NAME2 = [
 ["","low"]
,["","fixed"]
,["","high"]
];
}
